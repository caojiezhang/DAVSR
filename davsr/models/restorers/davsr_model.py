...
class DAVSRMODEL(BasicRestorer):
    """DAVSRNet model for video super-resolution.
    ...
    def train_step(self, data_batch, optimizer):
        ...
        if torch.isfinite(loss).all(): # may encounter NaN when using DCN
            # optimize
            optimizer['generator'].zero_grad()
            loss.backward()
            optimizer['generator'].step()
        else:
            print('--------skipping this batch due to NaN error')
    ...
    def forward_test(self,
        ...
        if self.test_cfg.get('normalize', True):

            w, h = lq.shape[3], lq.shape[4]
            W, H = w*4, h*4
            w1, h1 = math.ceil(lq.shape[3]/32) * 32, math.ceil(lq.shape[4]/32) * 32

            lq_tmp = torch.zeros(lq.shape[0], lq.shape[1], lq.shape[2], w1, h1).cuda()
            lq_tmp[:, :, :, :w, :h] = lq
            lq = lq_tmp
            del lq_tmp

        with torch.no_grad():
            num_frame_testing = self.test_cfg.get('num_frame_testing', 10)
            stride = num_frame_testing
            d_idx_list = list(range(0, 100, stride))
            ...
            for d_idx in d_idx_list:
                if d_idx == d_idx_list[-1]:
                    ...
                    gt_i = gt[:, 5*d_idx:(5*d_idx+5*num_frame_testing), ...]
                output_i = self.generator(lq_clip).cpu()

                if self.test_cfg.get('normalize', True):
                    output_i = output_i[:, :, :, :W, :H]
        ...
        if save_image:
            if output.ndim == 4:  
                ...
                if isinstance(iteration, numbers.Number):
                    ...
                    lq_save_path = osp.join(save_path, f'{img_name}_lq.png')
                    save_path = osp.join(save_path, f'{img_name}.png')
                else:
                    raise ValueError('iteration should be number or None, '
                                     f'but got {type(iteration)}')
                mmcv.imwrite(tensor2img(output), save_path)
                mmcv.imwrite(tensor2img(lq[:,0,...]), lq_save_path)
                ...
                for i in range(0, output.size(1)):
                    if isinstance(iteration, numbers.Number):
                        ...
                        save_path_i = osp.join(save_path, folder_name, f'{i:08d}.png')
                    else:
                        raise ValueError('iteration should be number or None, '
                                         f'but got {type(iteration)}')
